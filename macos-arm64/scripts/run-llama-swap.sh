#!/usr/bin/env bash
# Runs llama-swap natively (Metal, via the llama-server binary it manages),
# bound to 127.0.0.1 so that only the hermes container (via
# host.docker.internal) and this machine can reach it.
set -euo pipefail
...
if [ ! -f data/models.yaml ]; then
  echo "data/models.yaml not found — copy it from config/models.yaml.example first:" >&2
  echo "  mkdir -p data && cp config/models.yaml.example data/models.yaml" >&2
  exit 1
fi
...
# LLAMA_SERVER_BIN, MODEL_FILE and LLAMA_CTX_SIZE are already exported above
# (set -a) for models.yaml's ${env.*} macros to pick up.
exec "${LLAMA_SWAP_BIN}" \
