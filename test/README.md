...
| `smoke-vps.sh` | The **real** `linux-x86_64-vps/docker-compose.yml` `llama-server` service starts healthy, lists its model over `/v1/models`, and answers a real `/v1/chat/completions` request — with a tiny model swapped in for speed | Yes | Docker |
| `smoke-macos.sh` | The same wiring (`download-prebuilt-llama-server.sh` → `run-llama-server.sh` → a real completion), natively with Metal | No — needs an actual Apple Silicon Mac | macOS, Apple Silicon |
