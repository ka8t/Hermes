...
# itself fail silently (found live, 2026-09-10: TELEGRAM_BOT_TOKEN blank
# during a legitimate token-rotation window made this script exit 2 on
# every run for ~35 minutes, visible only in the local systemd journal,
# nobody watching). Track every run's outcome in a state file so a
# failure is discoverable somewhere other than journalctl, and (on
# systemd — see silent-failure-watchdog-alert.service.example) trigger an
# immediate `wall` broadcast plus a login-banner warning on every SSH
# session until this script succeeds again. LAST_ERROR_MSG is set right
# before each known failure's `exit`; the ERR/EXIT trap combo below also
# catches any *unforeseen* crash (a python traceback, a future bug) the
# same way, so this isn't limited to the failure modes anticipated today.
...
# source — ${LAST_ERROR_MSG} can contain quotes/apostrophes (real error
