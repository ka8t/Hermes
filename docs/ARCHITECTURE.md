...
| Hardware auto-detection (CPU/GPU) + mandatory inference benchmark | #11-#15, #27 | CPU thread auto-detection live in `provision.sh`, documented in `shared/hardware-sizing.md` (#15, partial); GPU detection (#13), native-path/macOS thread verification (#12/#14), and the mandatory real-throughput benchmark (#27) are not yet built |
