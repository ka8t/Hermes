...
| Agentic goal-drift root cause (PRIORITY) | #37, #48 | Root-caused and confirmed live via a real end-to-end Telegram test (#46, both Mac/Metal and VPS/CPU): the array-serialization bug now confirmed general across three tools (`delegate_task`, `clarify`, `skill_manage`), and a new, more serious failure mode found on the VPS run — the model fabricating a success narrative after every real tool call failed (#48). Two defense-in-depth mitigations live (`agent.run_budget_seconds: 3600`, regression cases recorded on #29). Neither is the fix — the durable fix is #28-#32's model evaluation, still unimplemented |
