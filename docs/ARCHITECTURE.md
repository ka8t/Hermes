...
| Hardware auto-detection (CPU/GPU) + mandatory inference benchmark | #11-#15 | CPU thread auto-detection live in `provision.sh`; the mandatory real-throughput benchmark (#27) is now live (`scripts/verify-inference.sh`, both platforms) — documented in `shared/hardware-sizing.md` (#15, partial). GPU detection (#13) and native-path/macOS thread verification (#12/#14) are not yet built |
