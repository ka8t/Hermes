...
| Enterprise RAG (Google Drive, Confluence, SharePoint) | #21-#26 | Specced only, nothing built. Model distillation was considered alongside this and explicitly rejected as out of scope — see #21 |
| Model/config evaluation harness | not yet filed | Specced in conversation: BFCL (Berkeley Function-Calling Leaderboard) for model tool-calling reliability, `llama-bench` (bundled with llama.cpp) for hardware/config throughput, results in an internal database, broad model list, continuous (CI-style) re-evaluation, exposed in this repo for an admin to reproduce on their own hardware |
| Instant message-received acknowledgment | not yet filed | Specced in conversation: a `pre_gateway_dispatch` hook sending an immediate ack independent of backend LLM speed, since Hermes's native `typing_indicator`/`long_running_notifications` can't show anything during pure prefill (no bytes come back from the model until the first token) |
