...
| Model/config evaluation harness | #28-#32 | BFCL path (#29) implemented and containerized (#51 — the venv/`bfcl-eval` install runs entirely inside `eval/Dockerfile`, not on the host, after an iCloud Drive file-eviction bug on macOS ruled out a host venv). **Real, complete accuracy scores obtained** (2026-09-04, this Mac, M1/Metal): `simple_python` 400/400 cases, **54.75%**; `parallel` 200/200 cases, **52.50%** (#42, closed). `multi_turn` categories were stopped mid-run by design (too slow to finish in reasonable time on a single non-batching llama-server instance) and intentionally left unscored — an incomplete run isn't a meaningful percentage. This repo's own regression cases (`eval/regression-goal-drift.sh` for #37, `eval/regression-hallucinated-success.sh` for #48) supplement BFCL's generic datasets with checks specific to this repo's actual failure history. `llama-bench -o sql` for hardware/config throughput (#30) and continuous re-evaluation (#31) remain specced only |
...
| Agentic goal-drift and hallucinated success | #37, #48 | **Both closed, 2026-09-04, with real fixes** — not full corrections of the underlying model capability limitation (still tracked as a reason to prioritize #28-#32's model evaluation), but durable, automated regression gates plus one genuine behavioral fix. #37: `eval/regression-goal-drift.sh` runs this repo's real failure prompt and checks the model's first tool call; not deterministic (6 runs split 4 `FAIL`/2 `PASS`) — ordinary sampling variance, not a 100%-reproducible bug. #48: `eval/regression-hallucinated-success.sh` checks whether a real tool-call failure gets narrated as success; **fixed** by a mandatory instruction baked into `docker/SOUL.md` (always injected into every system prompt, unlike a discretionary skill, which tested ineffective — see `shared/model-notes.md`) — verified live, 2/2 honest-failure runs with the fix vs. 2/2 false-success runs without it. `agent.run_budget_seconds: 3600` remains as defense-in-depth for both. |
| `web_search` tool-name collision with llama-server | #50 | **Fixed.** llama-server has hardcoded, name-based special handling for any tool literally called `web_search` — confirmed via `strings` on the compiled binary and direct A/B testing, not assumed. Fixed by removing the `limit` parameter from this repo's `web_search` schema entirely (`docker/patch-web-search-schema.py`), keeping the tool's name (several bundled skills reference it by that exact name) rather than renaming it. Two earlier, wrong theories (an `additionalProperties: false` guess, then a required-vs-optional guess) were each corrected after actually capturing/testing the real request — see `shared/model-notes.md` |
