...
2. **Ask, in a single plain chat message, only the questions still open**,
   from this list (skip any already answered). Write the questions directly
   as the text of your reply. Do not call a tool or function to do this —
   this skill is guidance for you to follow, not a callable function, and
   there is no tool by this or any similar name. On messaging platforms,
   batching the questions into one plain message also avoids asking them
   one at a time.
...
- Do not call any tool or function to ask these questions — see step 2. A
  plain chat message works every time and matches this skill's own
